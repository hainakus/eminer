...
func (e *Event) ExecutionCompleted() bool {
	var readStatus C.cl_int

	C.clGetEventInfo(e.clEvent, C.CL_EVENT_COMMAND_EXECUTION_STATUS, C.size_t(unsafe.Sizeof(readStatus)),
		unsafe.Pointer(&readStatus), nil)

	return readStatus == C.CL_COMPLETE
}
