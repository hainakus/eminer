...
func newMemObject(mo C.cl_mem, size uint64) *MemObject {
...
func (ctx *Context) CreateProgramWithBinary(data []byte, device *Device) (*Program, error) {
	var err C.cl_int

	numDevices := 1

	c_lengths := make([]C.size_t, numDevices)
	c_binaries := make([]*C.uchar, numDevices)

	c_lengths[0] = C.size_t(len(data))

	c_binaries[0] = (*C.uchar)(C.malloc(c_lengths[0]))
	C.memcpy(unsafe.Pointer(c_binaries[0]), unsafe.Pointer(&data[0]), c_lengths[0])
	defer C.free(unsafe.Pointer(c_binaries[0]))

	clProgram := C.clCreateProgramWithBinary(ctx.clContext, C.cl_uint(numDevices), &device.id, &c_lengths[0], &c_binaries[0], nil, &err)
	if err != C.CL_SUCCESS {
		return nil, toError(err)
	}
	if clProgram == nil {
...
func (ctx *Context) CreateBufferUnsafe(flags MemFlag, size uint64, dataPtr unsafe.Pointer) (*MemObject, error) {
	var err C.cl_int
...
func (ctx *Context) CreateEmptyBuffer(flags MemFlag, size uint64) (*MemObject, error) {
	return ctx.CreateBufferUnsafe(flags, uint64(size), nil)
}

func (ctx *Context) CreateEmptyBufferFloat32(flags MemFlag, size uint64) (*MemObject, error) {
	return ctx.CreateBufferUnsafe(flags, 4*size, nil)
}

func (ctx *Context) CreateBuffer(flags MemFlag, data []byte) (*MemObject, error) {
	return ctx.CreateBufferUnsafe(flags, uint64(len(data)), unsafe.Pointer(&data[0]))
}
